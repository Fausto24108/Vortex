...
async function autenticar(req, res, next) {
  try {
    ...
    if (!token) {
      return res.status(401).json({ mensaje: 'No hay sesión iniciada' })
    }
...
app.post('/api/auth/register', async (req, res) => {
  try {
    ...
    const resultado = await users.insertOne(nuevoUsuario)
    ...
    res.status(201).json({
      ...
      usuario: {
        ...
        biografia: usuarioCreado.biografia
      }
    })
  } catch (error) {
    console.error(error)
